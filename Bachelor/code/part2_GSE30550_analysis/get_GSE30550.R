...
keep_genes <- rowSums(expression_data>=1) >= 0.5*ncol(expression_data)
...
# update symbols_df and expression_data
symbols_df <- symbols_df[symbols_df$ENTREZID %in% string_mapped$ENTREZID,]
expression_data <- expression_data[rownames(expression_data) %in% symbols_df$ENTREZID,]
