...
###########################################################
def network_modeling(s_array, S, edge_relations, output_dir,
                     steps = 1000):
    '''
    网络总体建模
    ...
    print("---------------Starting modeling gene regulatory networks---------------")
    # 节点数
    nodes = S.shape[0]
    ...
    for idx, s in enumerate(s_array):
        ...
        general_noise_cov_matrix = generate_gaussian_noise()
        ...
        for step in range(steps):
            ...
            noise_cov_matrix = generate_gaussian_noise()
    ...
    print("-----Index 1:The standard deviation (SD) of y at different s values has been calculated successfully!")
    ...
    print("-----Index 2: The standard deviation (SD) of nodes' expression at different s values has been calculated successfully!")
    ...
    print("-----Index 3: The correlation (PCC) of edges between nodes at different s values has been generated successfully!")
    ...
    print("---Simulation data for every node's expression at different s values has been generated successfully!")
    ...
    print("---The standard deviation (SD) of simulation data at different s values has been calculated successfully!"+"\n")
...
def calculate_FT(case_path, ref_path, output_dir):
    ...
    print("---------------Starting calculating every node's fluctuation index: FT---------------")
    # 打开病例样本的不同s值表达文件
    ...
    print("------The FT values of simulation data at different s values have been calculated successfully!"+"\n")
...
def calculate_w(node_relations_path, node_pcc_path,case_std_path, output_dir):
    ...
    : param output_dir: 计算结果保存文件夹
    '''
    print("---------------Starting calculating causality strength index between nodes: w---------------")
    # 打开节点pearson相关系数文件
    ...
    print("-----The w values of simulation data at different s values have been calculated successfully!"+"\n")
...
###########################################################
def calculate_local_E(node_relations_path,FT,w):
    '''
    计算局部因果网络的RF-SCNE值
    : param node_relations_path: 节点关系文件路径
...
###########################################################
def calculate_global_E(node_relations_path,FT_path,w_path,output_dir):
    '''
    计算全局因果网络的RF-SCNE值
    : param node_relations_path: 节点关系文件路径
    : param FT_path: FT值文件存放路径
    ...
    print("---------------Starting calculating global causality network entropy: SI-ICNE---------------")
    # 打开FT值文件
    FT_df = pd.read_csv(FT_path, index_col=0)
    # 打开w值文件
    w_df = pd.read_csv(w_path, index_col=0)
    # 设置RF-SCNE值空存储
    total_E = np.zeros(w_df.shape[0])
    for i in range(w_df.shape[0]):
        FT = FT_df.iloc[i]
        w = w_df.iloc[i]
        H_k = calculate_local_E(node_relations_path=node_relations_path,FT=FT,w=w)
    ...
    print("-----Global SI-ICNE at different s values has been calculated successfully!"+"\n")
